...
let
  ...
  homeDirectory = config.home-manager.users.${config.husjon.user.username}.home.homeDirectory;

  nvidia_env =
    if config.husjon.graphics.manufacturer == "nvidia" then
      [
...
in
{
  config = lib.mkIf (config.husjon.graphics.window_manager == "hyprland") {
