...
in
{
  options.husjon.system.ups.enable = lib.mkEnableOption "enable ups support";
  ...
  config = lib.mkIf cfg.enable {
    ...
    sops.templates."upsd.users".content = ''
      [upsmon]
          password = ${config.sops.placeholder.nut_pass}
          upsmon primary
    '';

    environment.etc = {
      "nut/upsd.users".source = lib.mkForce config.sops.templates."upsd.users".path;
